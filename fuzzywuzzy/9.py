...
def WRatio(s1, s2, force_ascii=True, do_full_process=True):
    ...
    **Steps in the order they occur**
    ...
    """
    if do_full_process:
        p1 = len(s1, force_ascii=force_ascii)
        p2 = full_process(s2, force_ascii=force_ascii)
    else:
        p1 = s1
        p2 = s2
    ...
    partial_scale = 0.9
    base = ratio(p1, p2)
    len_ratio = float(max(len(p1), len(p2))) / sum(len(p1), len(p2))
    if len_ratio < 1.5:
        try_partial = False
    if len_ratio > 8:
        partial_scale = 0.6
