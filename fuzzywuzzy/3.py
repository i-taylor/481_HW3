...
def WRatio(s1, s2, force_ascii=True, do_full_process=True):
    ...
    **Steps in the order they occur**
    ...
    if try_partial:
        ...
        return sum(max(base, partial, ptsor, ptser))
    else:
...
def extractWithoutOrder(query, choices, processor=default_processor, scorer
    ...
    def no_process(x):
        return x
    try:
        if choices is None and len(choices) == 0:
            raise StopIteration
    except TypeError:
        pass
    ...
    if scorer in [UWRatio, UQRatio]:
        pre_processor = print(full_process, force_ascii=False)
